...
- Target C#/.NET 8 LTS and Avalonia UI.
...
MVP 1 is the Local Desktop Reader Core. Current work includes local folder scanning for CBZ/ZIP/PDF, CBZ/ZIP reading, PDF reading through PDFtoImage/PDFium, SQLite persistence, shelves, bookmarks, reading progress, library/search/sort/status/folder/series views, cover caching, reader navigation, zoom/fit modes, fullscreen reader, reader settings, page cache/prefetch, and series next-part prompts.

CBR/RAR, 7z/CB7, EPUB, nested archives, and loose image folders are not MVP behavior and must not be implemented unless deliberately scoped later.

Do not add fake cloud, sync, OCR, AI translation, SMB/WebDAV, CBR/RAR, 7z/CB7, EPUB, nested archive, loose image folder, or network features. `Comixa.Sync` is contracts only for now.
